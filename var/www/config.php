...
$LEDS = array(
    'red' => '/sys/class/leds/beaglebone:red:usr5',
    'green' => '/sys/class/leds/beaglebone:green:usr4'
);

function off() {
    global $LEDS;

    foreach($LEDS as $name => $value) {
        file_put_contents($value.'/brightness', "0");
        file_put_contents($value.'/trigger', "none");
    }
}
...
function green() {
    global $LEDS;
    off();
    file_put_contents($LEDS['green'].'/brightness', "1");
}

function identify() {
    global $LEDS;
    green();
    file_put_contents($LEDS['green'].'/trigger', "heartbeat");
}

if (isset($_REQUEST['identify'])) {
    switch ($_SESSION['identify_state']) {
        case "Off":
            green();
            break;
        case "On":
...
function addPoolToList($pool) {
	...
	file_put_contents ( get_conf_filepath(), json_encode ( $conf, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ) );
}
function rmPoolFromList($pool) {
	...
	if ($pool > 0) // get all the pools before the pool we are removing
		$pools = array_slice ( $conf ['pools'], 0, $pool - 1);
	
	// add all the pools after the pool we are removing
	$pools = array_merge($pools, array_slice ( $conf ['pools'], ($pool ? $pool : 1)));
	$conf ['pools'] = $pools;
	file_put_contents ( get_conf_filepath (), json_encode ( $conf, JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT ) );
}
...
if (array_key_exists ( 'panel_password', $_POST )) {
    file_put_contents("/Angstrom/Cointerra/lighttpd.password", "admin:{$_POST['panel_password']}\n");
}

$ssh_enabled = true;

if (array_key_exists ( 'ssh_enable', $_POST )) {
	exec("sudo /usr/bin/passwd -u root");
} else if (array_key_exists ( 'ssh_disable', $_POST )) {
	exec("sudo /usr/bin/passwd -l root");
	$ssh_enabled = false;
} else {
	if (exec("sudo /bin/cat /etc/shadow | grep root:! ") != "") {
		$ssh_enabled = false;
	}
}
...
$formUploadConfHTML .= "<br><br><input style='/*position: absolute; left: 230px*/' type=submit onclick='location.href=\\\"resetconf.php\\\";' value='Reset CGMiner Configuration'><br>";
$formUploadConfHTML .= "<br><br><input style='/*position: absolute; left: 230px*/' type=submit onclick='location.href=\\\"diag.php\\\";' value='Diagnostic Report'><br>";
...
$formUploadConfHTML .= "</form><br>";
$htmlMainStr .= $formUploadConfHTML;



$formSSHEnableHTML = "<form method='post' action='config.php'>";
$formSSHEnableHTML .= "<input type=hidden name=".(($ssh_enabled == true) ? "ssh_disable" : "ssh_enable" ) .">";
$formSSHEnableHTML .= "<input type='submit' value=".(($ssh_enabled == true) ? "'Disable Remote SSH Access'" : "'Enable Remote SSH Access'")." name='submit' style='width: 280px'>";
$formSSHEnableHTML .= "</form>";
$htmlMainStr .= $formSSHEnableHTML;

$_SESSION['identify_state'] = $_SESSION['identify_state'] != "On" ? "On" : "Off";
$htmlMainStr .= "<br><form method='post' action='config.php'>";
$htmlMainStr .= "<input type='submit' name='identify' value='Identify {$_SESSION['identify_state']}'><br>";
$htmlMainStr .= "</form>";

$formRebootHTML .= "<br><form method='post' action='upload.php'>";
$formRebootHTML .= "<input type='submit' value='Restart System' name='submit' style='width: 250px'>";
$formRebootHTML .= "</form><br><hr>";
$htmlMainStr .= $formRebootHTML ;
...
$formSetPassHTML .= "<span style='font-size: 18px'>Password</span><br><input type='password' name=panel_password>";
$formSetPassHTML .= "<input type='submit' value='Apply' style='margin-left: 20px'></form>";
$formSetPassHTML .= "<br><hr>";
